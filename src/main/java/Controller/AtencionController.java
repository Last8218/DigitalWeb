...
public class AtencionController extends HttpServlet {
    ...
    private void agregarAtencion(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        try {
            ...
            String estado = "";
            if (data.has("estado")) {
                String estadoRaw = data.get("estado").getAsString().trim();
                if (!estadoRaw.isEmpty()) {
                    estado = estadoRaw.substring(0, 1).toUpperCase() + estadoRaw.substring(1).toLowerCase();
                }
            }
            System.out.println("Esado recibido: " + estado);
            ...
            //Buscar la solicitud que se está modificando
            SolicitudDTO solicitudDTO = SolicitudService.buscarSolicitudPorId(idSolicitud);
            solicitudDTO.setEstado(estado);

            SolicitudService.actualizarSolicitud(solicitudDTO);
